...
exports.get_status = async (): Promise<AxiosResponse<Status, any>> => {
...
exports.lock_cmd = async (): Promise<AxiosResponse<any, any>> => {
  return await operate_sesame('locked')
}
...
const operate_sesame = async (lockType: LockType) => {
  ...
  const sign: Buffer = generateRandomTag(KEY_SECRET_HEX || '');
...
const setCmdAndHistory = (cmd: LockType) => {
